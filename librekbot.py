...
from librus_tricks import create_session, SynergiaClient, authorizer
                ...
                if message.get('message'):
                    ...
                    if message['message'].get('text'):
                        ...
                        # elif(msg.lower() == "wszystkie_zmiany"):
                            # for news in session.news_feed():
                                # send_message(recipient_id, f'{news.content}')
                        elif(msg.lower().replace('!', '').replace('.', '') in ['ostatnie zmiany' , 'zmiany', 'last changes', 'changes'] or "zmiany" in msg.lower() or "changes" in msg.lower()):
                        ...
                        elif(msg.lower().replace('!', '').replace('.', '') in ['hi' , 'hello', 'hej', 'siema', 'good morning', 'greetings']):
                            ...
                            send_message(recipient_id, "Zostałem stworzony, żebyś mógł łatwiej dowiedzieć się o zmianach w planie :)")
                        elif(msg.lower().replace('!', '').replace('.', '') in ['dzięki' , 'dzieki', 'dziekuje', 'dziękuję', 'thanks', 'thank\'s', 'thank you']):
                            wiad = random.choice(["Zawsze do usług!", "Nie ma sprawy", "Miło było Ci pomóc!"])
                            send_message(recipient_id, wiad)
                        elif(msg.lower().replace('!', '').replace('.', '') in ['bye' , 'do zobaczenia', 'goodbye', 'see ya', 'see ya later', 'dobranoc', 'good night']):
                            wiad = random.choice(["Zawsze do usług!", "Nie ma sprawy!", "Miło było Ci pomóc!"])
                            send_message(recipient_id, wiad)
                        else:
                            first = random.choice(["Niestety, ale tym razem nie udało mi się zrozumieć Twojej wiadomości.", "Nie wiem, jak Ci odpowiedzieć, może spróbujesz czegoś innego?", "Nie zrozumiałem Cię, czy możesz napisać inaczej?"])
                ...
                with open('recipients.txt', 'r') as filehandle:
                    for line in filehandle:
                        ...
                        for line in news.content.split("\n"):
                            otherClass = False
                            if("3f" not in line.lower() and "wszyscy" not in line.lower() and "wszystkie" not in line.lower() and "każda" not in line.lower()):
                                for i in range (1,4):
